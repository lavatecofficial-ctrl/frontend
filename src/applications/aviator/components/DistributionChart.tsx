...
export default function DistributionChart({ history }: DistributionChartProps) {
  const distributionData = useMemo(() => {
    if (history.length === 0) {
      return [
        { label: '1.00-1.50', value: 0, color: '#ef4444' },
        { label: '1.51-2.00', value: 0, color: '#f59e0b' },
        { label: '2.01-3.00', value: 0, color: '#22c55e' },
        { label: '3.00-10.00', value: 0, color: '#3b82f6' },
        { label: '10 o más', value: 0, color: '#8b5cf6' },
      ];
    }

    const last100 = history.slice(-100);
    const ranges = [
      { label: '1.00-1.50', min: 1.00, max: 1.50, color: '#ef4444' },
      { label: '1.51-2.00', min: 1.51, max: 2.00, color: '#f59e0b' },
      { label: '2.01-3.00', min: 2.01, max: 3.00, color: '#22c55e' },
      { label: '3.00-10.00', min: 3.00, max: 10.00, color: '#3b82f6' },
      { label: '10 o más', min: 10.00, max: Infinity, color: '#8b5cf6' },
    ];
